...
class YouTube {
    ...
    private fun collectPlayableUrls(response: PlayerResponse): List<String> {
        val urls = mutableListOf<String>()
        response.streamingData?.formats?.mapNotNull { it.url }?.let { urls.addAll(it) }
        response.streamingData?.adaptiveFormats?.mapNotNull { it.url }?.let { urls.addAll(it) }
        response.streamingData?.hlsManifestUrl?.let { urls.add(it) }
        return urls.filter { it.isNotEmpty() }
    }

    private fun hasPlayableUrls(response: PlayerResponse): Boolean = collectPlayableUrls(response).isNotEmpty()

    private suspend fun iosPlayerResponse(
        videoId: String,
        tempRes: PlayerResponse,
    ): PlayerResponse? {
        if (tempRes.playabilityStatus.status != "OK") return null

        if (hasPlayableUrls(tempRes)) {
            val randomUrl = collectPlayableUrls(tempRes).randomOrNull() ?: return null
            if (!is403Url(randomUrl)) {
                Logger.d(TAG, "iOS direct stream URL found")
                return tempRes
            }
        }

        for (instance in listPipedInstances) {
            val piped = pipeStream(videoId, instance).getOrNull() ?: continue
            val pipedFormats = (piped.audioStreams + piped.videoStreams).toListFormat()
            val streamingData = tempRes.streamingData
            val merged =
                tempRes.copy(
                    streamingData =
                        if (streamingData != null) {
                            streamingData.copy(
                                formats =
                                    (streamingData.formats.orEmpty() + pipedFormats)
                                        .filter { it.url.isNullOrEmpty().not() },
                                adaptiveFormats =
                                    (streamingData.adaptiveFormats + pipedFormats)
                                        .filter { it.url.isNullOrEmpty().not() },
                                hlsManifestUrl = piped.hls ?: streamingData.hlsManifestUrl,
                            )
                        } else {
                            PlayerResponse.StreamingData(
                                hlsManifestUrl = piped.hls,
                                formats = pipedFormats,
                                adaptiveFormats = emptyList(),
                                expiresInSeconds = 21_600,
                            )
                        },
                )
            val randomUrl = collectPlayableUrls(merged).randomOrNull() ?: continue
            if (!is403Url(randomUrl)) {
                Logger.d(TAG, "iOS Piped stream URL found via $instance")
                return merged
            }
        }
        return null
    ...
    suspend fun player(
        ...
        runCatching {
            ...
            val playerClient = if (isIosPlatform()) IOS else WEB_REMIX
            val tempRes =
                ytMusic
                    .player(
                        playerClient,
            ...
            val response =
                if (isIosPlatform()) {
                    iosPlayerResponse(videoId, tempRes)
                } else {
                    newPipePlayer(videoId, tempRes)
                }
            if (response != null) {
